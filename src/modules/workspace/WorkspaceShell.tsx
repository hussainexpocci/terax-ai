...
export function WorkspaceShell({ workspaceId: _workspaceId, active }: WorkspaceShellProps) {
  const {
    tabs,
    activeId,
    setActiveId,
  ...
  const switchWorkspaceEnvInner = useCallback(
  ...
  const openPreviewTabFn = useCallback(
    (url: string) => {
  ...
  const shortcutHandlers = useMemo<ShortcutHandlers>(
    () => ({
      ...
      'tab.newPreview': () => openPreviewTabFn(''),
  ...
  useEffect(() => {
    ...
    setLive({
      ...
      openPreview: (url: string) => {
        openPreviewTabFn(url);
        return true;
      },
    });
  }, [setLive, activeId, tabs, explorerRoot, home, openPreviewTabFn]);
    ...
    <TooltipProvider>
        <div className="relative flex h-screen flex-col overflow-hidden bg-background text-foreground">
          ...
          <StatusBar
            ...
            home={home}
            onCd={sendCd}
            onWorkspaceChange={switchWorkspaceEnvInner}
            onOpenMini={openMini}
